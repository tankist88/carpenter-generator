...
public class MethodsFormatUtil {
    ...
    private static Set<MethodSource> createCommonMethods(List<MethodExtInfo> allMethods) {
        ...
        for (MethodExtInfo extInfo : duplicateMethods) {
            ...
            String providerName = extInfo.createCommonMethodName() + CreateJavaClassesCommand.PROVIDER_POSTFIX;
            definition = definition.replace(TEST_ANNOTATION, TEST_ANNOTATION + "(" + CreateJavaClassesCommand.DATA_PROVIDER_PARAMETER + " = \"" + providerName + "\")");
    ...
    private static List<MethodExtInfo> createDataProviders(List<MethodExtInfo> allMethodsSortedList) {
        ...
        for (Map.Entry<MethodBaseInfo, List<MethodExtInfo>> entry : groupingMap.entrySet()) {
            ...
            String methodName = entry.getKey().getUnitName() + CreateJavaClassesCommand.PROVIDER_POSTFIX + "()";
            dataProviderBuilder
                    .append(TAB + CreateJavaClassesCommand.DATA_PROVIDER_ANNOTATION +"\n")
